...
def calculate(
    ...
    if rec["topMode"] == "measurement":
        ...
        top_reason = (
            f"Hibrit mod: BMI puanı ({fmt_num(round2(hybrid['bmiScore']))}) + "
            f"tahmini göğüs ({fmt_num(estimate['chest'])} cm, skor {fmt_num(round2(hybrid['measureScore']))}) "
            f"birlikte değerlendirildi.{brand_txt}"
        )
    ...
    if rec["bottomMode"] == "measurement":
        ...
        brand_txt = f" Marka referansı: {brand['nearest']} ({js_round(brand['strength'] * 100)}% etki)." if brand and brand["strength"] > 0 else ""
        bottom_reason = (
            f"Hibrit mod: BMI puanı ({fmt_num(round2(hybrid['bmiScore']))}) + {target_txt} "
            f"birlikte değerlendirildi.{brand_txt}"
        )

    result = {
        "topMain": top_row["label"],
        "topAlt": f"Alternatif: {top_alt['label']}",
        "bottomMain": bottom_row["label"],
        "bottomAlt": f"Alternatif: {bottom_alt['label']}",
        "topConfidence": f"Güven: {top_conf}",
        "bottomConfidence": f"Güven: {bottom_conf}",
        "topReason": top_reason,
        "bottomReason": bottom_reason,
        "input": input_data,
